...
<div class="flex h-full flex-col items-center justify-center gap-10 px-12 py-12">
  ...
  <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
    {#each [{ icon: '⇣', label: 'Clone a Repo', hint: 'from URL', action: onClone }, { icon: '📂', label: 'Open Local', hint: 'from disk', action: handleOpenLocal }, { icon: '+', label: 'Init New Repo', hint: 'git init', action: handleInitRepo }, { icon: '⊞', label: 'Multi-Repo', hint: 'bulk ops', action: handleOpenMultiRepo }] as item}
      <button
        class={cn(
